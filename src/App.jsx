...
function Welcome({ onCreate, onJoin }) {
  return <main className="welcome page-shell">
    ...
    <div className="welcome__content">
      ...
      <p className="eyebrow">A private place for your people</p>
...
function HuddleForm({ mode, form, setForm, onNext, onBack, error, loading }) {
  ...
  return <main className="form-page page-shell">
    <Back onClick={onBack} />
    <div className="form-page__intro"><span className="step">01 / 02</span><h1>{isCreate ? 'Create your Huddle' : 'Join your Huddle'}</h1><p>{isCreate ? 'A private place for your family to stay connected.' : 'Enter the details shared by your family.'}</p></div>
...
function Profile({ form, setForm, onContinue, onBack, loading }) {
  return <main className="form-page page-shell"><Back onClick={onBack} /><div className="form-page__intro"><span className="step">02 / 02</span><h1>Welcome to Huddle</h1><p>What should your family call you?</p></div><form className="form-stack" onSubmit={(event) => { event.preventDefault(); onContinue() }}><Field label="Your name" value={form.profileName} onChange={(profileName) => setForm((current) => ({ ...current, profileName }))} placeholder="e.g. Jamie" autoComplete="name" /><Button type="submit" disabled={loading}>{loading ? 'Joining...' : 'Continue'} <span>&#8594;</span></Button></form></main>
}

function Permission({ onEnable, onSkip, error, loading }) {
  return <main className="permission page-shell"><div className="permission__mark"><img src={iconBackground} alt="Huddle" /></div><span className="eyebrow">Your privacy, your choice</span><h1>Share your location</h1><p>Your Huddle members will be able to see your location on the family map.</p><p className="secondary-copy">You can stop sharing anytime.</p>{error && <p className="error-text" role="alert">{error}</p>}<div className="permission__actions"><Button onClick={onEnable} disabled={loading}>{loading ? 'Locating...' : 'Enable Location'} <span>&#8594;</span></Button><Button variant="quiet" onClick={onSkip}>Not now</Button></div></main>
}

function HuddleMap({ huddle, userId, profileName, location, sharing, onToggle, onLocate, onLeave }) {
  const [menuOpen, setMenuOpen] = useState(false)
  return <main className="map-page"><MapView members={huddle.members} home={huddle.home} currentUserId={userId} currentLocation={location} /><header className="map-header"><Brand compact /><div><strong>{huddle.name}</strong><span>{Object.keys(huddle.members || {}).length} {Object.keys(huddle.members || {}).length === 1 ? 'member' : 'members'}</span></div><button className="icon-button" onClick={() => setMenuOpen((open) => !open)} aria-label="Open menu">&#8942;</button></header><button className="locate-button" onClick={onLocate} aria-label="Locate me"><span>&#9678;</span> Locate</button>{menuOpen && <div className="map-menu"><div className="map-menu__profile"><span className="avatar">{profileName?.[0]?.toUpperCase()}</span><div><strong>{profileName}</strong><span>{sharing ? 'Location sharing on' : 'Location sharing off'}</span></div></div><button className="sharing-toggle" onClick={onToggle}><span className={`status-dot ${sharing ? 'status-dot--live' : ''}`} />{sharing ? 'Stop sharing' : 'Share location'}</button><button className="menu-link" onClick={onLeave}>Leave Huddle</button></div>}</main>
}
...
export default function App() {
  ...
  const toProfile = () => { if (!form.name.trim() || form.password.length < 6 || (mode === 'create' && !form.home)) { setError(mode === 'create' ? 'Choose your Home by tapping the map or selecting Use current.' : 'Add your Huddle name and a password of at least 6 characters.'); return } setError(''); setScreen('profile') }
  ...
  if (screen === 'profile') return <Profile form={form} setForm={setForm} onContinue={finishProfile} onBack={() => setScreen(mode)} loading={loading} />
  if (screen === 'permission') return <Permission onEnable={enableLocation} onSkip={() => setScreen('map')} error={error} loading={loading} />
  return <HuddleMap huddle={huddle} userId={user.uid} profileName={form.profileName} location={location} sharing={sharing} onToggle={toggleSharing} onLocate={() => location && setLocation({ ...location })} onLeave={leave} />
}
